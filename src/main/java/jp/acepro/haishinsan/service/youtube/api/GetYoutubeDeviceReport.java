...
package jp.acepro.haishinsan.service.youtube.api;
...
@Slf4j
public class GetYoutubeDeviceReport {

	public String propFileName;
	public String googleAccountId;
	public List<Long> campaignIdList = new ArrayList<Long>();
	public List<YoutubeDeviceReportDto> youtubeDeviceReportDtoList = new ArrayList<YoutubeDeviceReportDto>();
	...
	public void runExample(AdWordsServicesInterface adWordsServices, AdWordsSession session)
		...
		ReportQuery query = new ReportQuery.Builder()
				.fields("CampaignId", "CampaignName", "Date", "Device", "Impressions", "Clicks", "Cost", "VideoViews",
						"VideoViewRate", "AverageCpv")
				.from(ReportDefinitionReportType.CAMPAIGN_PERFORMANCE_REPORT).where("CampaignId").in(arr)
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				YoutubeDeviceReportDto youtubeDeviceReportDto = new YoutubeDeviceReportDto();
				youtubeDeviceReportDto.setCampaignId(Longs.tryParse(values.get(0)));
				youtubeDeviceReportDto.setCampaignName(values.get(1));
				String date = StringFormatter.dateFormat(values.get(2));
				youtubeDeviceReportDto.setDate(date);
				String devictTypeText = values.get(3);
				log.debug("devictTypeText : {}", devictTypeText);
				if (devictTypeText.startsWith("Computer")) {
					youtubeDeviceReportDto.setDeviceType(GoogleDeviceType.PC.getValue());
				}
				if (devictTypeText.startsWith("Mobile")) {
					youtubeDeviceReportDto.setDeviceType(GoogleDeviceType.MOBILE.getValue());
				}
				if (devictTypeText.startsWith("Tablet")) {
					youtubeDeviceReportDto.setDeviceType(GoogleDeviceType.TABLET.getValue());
				}
				youtubeDeviceReportDto.setImpressions(Longs.tryParse(values.get(4)));
				youtubeDeviceReportDto.setClicks(Longs.tryParse(values.get(5)));
				youtubeDeviceReportDto.setCosts(Longs.tryParse(values.get(6)).doubleValue() / 1000000);
				youtubeDeviceReportDto.setVideoViews(Longs.tryParse(values.get(7)));
				youtubeDeviceReportDto.setVideoViewRate(Doubles.tryParse(values.get(8)));
				youtubeDeviceReportDto.setAvgCpv(Doubles.tryParse(values.get(9)));

				youtubeDeviceReportDtoList.add(youtubeDeviceReportDto);
				log.debug("YoutubeDeviceReportDto : {}", youtubeDeviceReportDto.toString());
			}
