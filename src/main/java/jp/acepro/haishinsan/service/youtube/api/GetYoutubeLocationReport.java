...
@Slf4j
public class GetYoutubeLocationReport {

	public String propFileName;
	public String googleAccountId;
	public List<Long> campaignIdList = new ArrayList<Long>();
	public List<YoutubeLocationReportDto> youtubeLocationReportDtoList = new ArrayList<YoutubeLocationReportDto>();
	...
	public void runExample(AdWordsServicesInterface adWordsServices, AdWordsSession session) throws ReportDownloadResponseException, ReportException, IOException {
		...
		ReportQuery query = new ReportQuery
				.Builder()
				.fields("CampaignId", "CampaignName", "Date", "CountryCriteriaId", "CityCriteriaId", "Impressions", "Clicks", "Cost", "VideoViews", "VideoViewRate", "AverageCpv")
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				YoutubeLocationReportDto youtubeLocationReportDto = new YoutubeLocationReportDto();
				youtubeLocationReportDto.setCampaignId(Longs.tryParse(values.get(0)));
				youtubeLocationReportDto.setCampaignName(values.get(1));
				String date = StringFormatter.dateFormat(values.get(2));
				youtubeLocationReportDto.setDate(date);
				youtubeLocationReportDto.setLocationId(Longs.tryParse(values.get(4)));
				youtubeLocationReportDto.setImpressions(Longs.tryParse(values.get(5)));
				youtubeLocationReportDto.setClicks(Longs.tryParse(values.get(6)));
				youtubeLocationReportDto.setCosts(Longs.tryParse(values.get(7)).doubleValue() / 1000000);
				youtubeLocationReportDto.setVideoViews(Longs.tryParse(values.get(8)));
				youtubeLocationReportDto.setVideoViewRate(Doubles.tryParse(values.get(9)));
				youtubeLocationReportDto.setAvgCpv(Doubles.tryParse(values.get(10)));
				
				youtubeLocationReportDtoList.add(youtubeLocationReportDto);
				log.debug("YoutubeLocationReportDto : {}", youtubeLocationReportDto.toString());
			}
