...
public class TwitterTemplateServiceImpl extends BaseService implements TwitterTemplateService {
	...
	public void createTemplate(TwitterTemplateDto twitterTemplateDto) {
		...
		// テンプレート名チェック
		TwitterTemplate existName = twitterTemplateCustomDao.selectByName(twitterTemplateDto.getTemplateName(),
		...
		// テンプレート優先順チェック
		TwitterTemplate existPriority = twitterTemplateCustomDao
				.selectByPriority(twitterTemplateDto.getTemplatePriority(), ContextUtil.getCurrentShopId());
		...
		// 都道府県非空チェック
		if (String.valueOf(TwitterLocationType.REGION.getValue()).equals(twitterTemplateDto.getLocation())
				&& twitterTemplateDto.getRegions().isEmpty() == true) {
			// 配信都道府県を選択してください。
	...
	public List<TwitterTemplateDto> templateList() {
		...
		List<TwitterTemplate> twitterTemplateList = twitterTemplateCustomDao
				.selectAll(ContextUtil.getCurrentShop().getShopId());
		List<TwitterTemplateDto> twitterTemplateDtoList = TwitterMapper.INSTANCE
				.tempListEntityToDto(twitterTemplateList);
