...
public class GetDeviceReport {
	...
	public List<GoogleDeviceReportDto> googleDeviceReportDtoList = new ArrayList<GoogleDeviceReportDto>();
	...
	public void runExample(AdWordsServicesInterface adWordsServices, AdWordsSession session)
		...
		ReportQuery query = new ReportQuery.Builder()
				.fields("CampaignId", "CampaignName", "Date", "Device", "Impressions", "Clicks", "Cost")
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				GoogleDeviceReportDto googleDeviceReportDto = new GoogleDeviceReportDto();
				googleDeviceReportDto.setCampaignId(Longs.tryParse(values.get(0)));
				googleDeviceReportDto.setCampaignName(values.get(1));
				String date = StringFormatter.dateFormat(values.get(2));
				googleDeviceReportDto.setDate(date);
				String devictTypeText = values.get(3);
				log.debug("devictTypeText : {}", devictTypeText);
				if (devictTypeText.startsWith("Computer")) {
					googleDeviceReportDto.setDeviceType(GoogleDeviceType.PC.getValue());
				}
				if (devictTypeText.startsWith("Mobile")) {
					googleDeviceReportDto.setDeviceType(GoogleDeviceType.MOBILE.getValue());
				}
				if (devictTypeText.startsWith("Tablet")) {
					googleDeviceReportDto.setDeviceType(GoogleDeviceType.TABLET.getValue());
				}
				googleDeviceReportDto.setImpressions(Longs.tryParse(values.get(4)));
				googleDeviceReportDto.setClicks(Longs.tryParse(values.get(5)));
				googleDeviceReportDto.setCosts(Longs.tryParse(values.get(6)).doubleValue() / 1000000);

				googleDeviceReportDtoList.add(googleDeviceReportDto);
				log.debug("GoogleDeviceReportDto : {}", googleDeviceReportDto.toString());
			}
