...
public class ImageUtil {
	...
	public String getImageBytes(MultipartFile multipartFile, int mediaType) throws IOException {
		...
		if (!validSize(size, mediaType)) {
			throw new BusinessException(ErrorCodeConstant.E00003, 
					String.valueOf(size/1000), String.valueOf(maxSize/1000));
		}

		// ファイルの拡張子チェック
		String contentType = multipartFile.getContentType();
		if (!validType(contentType, mediaType)) {
			throw new BusinessException(ErrorCodeConstant.E00004, 
					contentType.substring(contentType.indexOf("/") + 1), 
					contentTypeList.stream().map(s -> s.substring(s.indexOf("/") + 1)).collect(Collectors.joining(", ")));
		}
		...
		if (!validDimension(image, mediaType)) {
			String code = MediaType.of(mediaType).equals(MediaType.FACEBOOK) ? ErrorCodeConstant.E40004 : ErrorCodeConstant.E00005;
		...
		if ("image/gif".compareToIgnoreCase(contentType) == 0) {
			...
			if (res == 1) { // 毎秒フレーム数のチェックエラー
				throw new BusinessException(ErrorCodeConstant.E00006, 
						String.valueOf(gifProperty.framesPerSecond), String.valueOf(gifFrames));
